...
- 에이임팩트에서 당근으로 넘어갈 때는 별도로 닮은 당근을 겹치지 않고 밭의 다섯 번째 실제 당근 요소 자체를 클릭한다. 당근 전체를 확대하면 가느다란 잎과 뿌리 실루엣 사이로 배경이 남아 화면이 주황색으로 채워지지 않았으므로, 실제 당근이 살짝 뽑힌 뒤 그 요소의 주황 뿌리 면만 카메라 앞으로 확대하는 매크로 줌으로 정정한다. 화면이 완전히 주황색으로 덮인 뒤 실제 지도와 같은 도로선이 먼저 나타나고 지도가 열리며, 역방향은 당근 초입에서 위로 스크롤해 임계점을 넘을 때 같은 순서를 빠르게 되감는다.
