...
- 사용자는 D(3형태 미니멀)를 최종 favicon 방향으로 선택했다. 1차 원본의 비율과 색을 유지한 채 브라우저별 크기만 파생한다.
- D 시안에서 16 px·32 px·48 px PNG, multi-size ICO, 180 px Apple touch icon을 파생해 사이트 `<head>`에 연결했다.
- D 파비콘과 함께 브라우저 title은 설명형 문구 대신 심플한 `Kiwi's`를 사용한다. 역할과 검색용 설명은 이후 SEO·Open Graph 메타데이터에서 보완한다.
