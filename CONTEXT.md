...
- 2026-08-09 정정: 에이임팩트에서 당근으로 넘어갈 때 당근 뿌리만 늘어나는 방식보다, 실제 당근은 화면에 남겨 두고 그 지점을 중심으로 페이지 전체가 확대되어 방문자가 당근으로 빨려 들어가는 카메라 전환을 사용한다. 완전히 주황색이 된 프레임에서 세로 위치를 숨겨 바꾸고 당근 장면 전체를 같은 중심에서 축소해 드러낸다. 전환 중 추가 스크롤 입력을 막고 역방향도 동일한 카메라 문법을 사용한다.
