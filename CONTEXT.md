...
- 에이임팩트에서 당근으로 넘어갈 때는 별도로 닮은 당근을 겹치지 않고 밭의 다섯 번째 실제 당근 요소 자체를 클릭한다. 스크롤에 따라 천천히 커지는 방식은 어색하므로, 후반에 클릭 가이드를 보여주고 클릭하면 빠르게 화면을 채운 뒤 전환한다. 다음 동네 지도에서는 같은 작물 DOM 구조가 같은 프레임에서 줄어들고, 역방향은 당근 초입에서 위로 스크롤해 임계점을 넘을 때 빠르게 실행해 픽셀과 디자인 미스매치를 없앤다.
