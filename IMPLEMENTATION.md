...
- 에이임팩트 후반에는 별도의 전환용 도형을 겹치지 않고 밭의 다섯 번째 작물인 실제 당근 DOM을 키보드 접근 가능한 버튼으로 사용한다. 스크롤 진행률은 클릭 가이드의 노출만 제어한다. 클릭하면 당근 전체를 살짝 위로 당기는 반동 뒤, 그 요소의 실제 주황 뿌리 pseudo-element만 독립적으로 확대해 viewport를 완전히 덮는다. 확대 중 뿌리색을 원래 농장색에서 당근 브랜드 오렌지로 자연스럽게 연결한다.
- 화면이 가려진 프레임에서 당근 트랙 초입으로 즉시 스크롤하고, 동일한 `.crop.crop-1` 구조의 뿌리 면 위에 실제 지도와 같은 도로선 레이어를 먼저 노출한 뒤 뿌리를 빠르게 축소해 지도를 드러낸다. 당근 트랙 초입에서 위 방향 스크롤이 임계 진행률을 통과하면 이 흐름을 반대로 실행한다. 전환 중에는 재진입을 잠그고 `prefers-reduced-motion`에서는 지연을 최소화한다.
