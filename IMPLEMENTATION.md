...
- 에이임팩트 후반에는 밭의 다섯 번째 실제 당근 DOM을 키보드 접근 가능한 버튼으로 사용한다. 클릭하면 뿌리를 독립적으로 늘리지 않고 실제 당근 뿌리의 화면 좌표를 transform origin으로 삼아 sticky stage 전체를 확대한다. viewport와 당근 위치에서 가장 먼 모서리까지 계산한 동적 배율을 사용해 확대 완료 프레임이 주황색 뿌리 내부로 완전히 채워지도록 한다.
