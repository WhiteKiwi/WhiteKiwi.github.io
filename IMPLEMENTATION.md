...
- `fortune`은 셸의 같은 이름 명령처럼 실행할 때마다 한 문장을 무작위로 골라 말풍선과 작은 키위로 보여준다. 문장은 이 포트폴리오를 위해 직접 쓴 것만 사용한다. 셸에 설치된 fortune 데이터베이스는 제3자의 인용문 모음이라 저장소로 옮기지 않는다.
- 말풍선 테두리는 문자 수가 아니라 칸 수로 계산한다. 한글과 CJK는 모노스페이스에서 두 칸을 차지해 문자 수로 맞추면 테두리가 어긋난다.
