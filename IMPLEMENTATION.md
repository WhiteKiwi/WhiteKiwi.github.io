...
- 에이임팩트 후반에는 밭의 다섯 번째 실제 당근 DOM을 키보드 접근 가능한 버튼으로 사용한다. 작물이 충분히 자라고 전환 가능한 진행률에 도달하기 전에는 버튼을 disabled 상태로 두어 키보드 포커스 순서에서도 제외한다. 활성화 후 클릭하면 뿌리를 독립적으로 늘리지 않고 실제 당근 뿌리의 화면 좌표를 transform origin으로 삼아 sticky stage 전체를 확대한다. viewport와 당근 위치에서 가장 먼 모서리까지 계산한 동적 배율을 사용해 확대 완료 프레임이 주황색 뿌리 내부로 완전히 채워지도록 한다.
- 화면이 가려진 프레임에서 당근 트랙 초입으로 즉시 위치를 바꾸고, 동일한 `.crop.crop-1` 구조를 원점으로 크게 확대된 당근 stage를 원래 배율로 축소해 동네 지도를 드러낸다. 이 시간 동안 wheel·touchmove·스크롤 키의 기본 동작을 막아 프로그램이 바꾼 세로 위치를 사용자가 느끼지 못하게 한다. 당근 트랙 초입에서 위 방향 스크롤이 임계 진행률을 통과하면 흐름을 반대로 실행하고, `prefers-reduced-motion`에서는 지연을 최소화한다.
