...
- 에이임팩트 후반에는 밭의 다섯 번째 실제 당근 DOM을 키보드 접근 가능한 버튼으로 사용한다. 작물이 충분히 자라고 전환 가능한 진행률에 도달하기 전에는 버튼을 disabled 상태로 두어 키보드 포커스 순서에서도 제외한다. 클릭 가이드가 완전히 보이는 진행률에서는 하향 wheel·touch·스크롤 키가 다음 track으로 넘어가지 못하게 현재 위치를 고정해, 실제 당근 활성화만 당근 장면의 진입 트리거가 되게 한다. 활성화 후 클릭하면 스크롤 진행률을 더 읽어 애니메이션을 보간하지 않고, 실제 당근 뿌리의 화면 좌표를 transform origin으로 삼은 시간 기반 sticky stage 확대를 즉시 시작한다. viewport와 당근 위치에서 가장 먼 모서리까지 계산한 동적 배율을 사용해 완료 프레임이 주황색 뿌리 내부로 완전히 채워지도록 한다.
- 화면이 가려진 프레임에서 당근 트랙 초입으로 즉시 위치를 바꾸되 당근 stage는 처음부터 정상 배율로 둔다. transition curtain은 각 sticky stage 안에 복제하지 않고 두 track 바깥의 단일 fixed viewport 레이어로 둔다. 실제 뿌리와 같은 색으로 화면을 계속 덮은 상태에서 위치를 교체하고, 짧은 정지 뒤 봄 교정 전환처럼 위로 걷어 올리며 opacity를 낮춰 동네 지도를 드러낸다. 역방향은 당근 초입의 상향 wheel·touch·키 입력을 native scroll 전에 가로채 같은 fixed curtain으로 먼저 화면을 가리고, 정상 배율·안전한 진행률의 에이임팩트로 교체한 뒤 curtain만 걷는다. sticky stage에는 역방향 transform이나 커튼 상태 class를 남기지 않으며, 전환 종료·중단 시 fixed 레이어와 확대 class를 한 번에 초기화한다. `prefers-reduced-motion`에서는 지연을 최소화한다.
