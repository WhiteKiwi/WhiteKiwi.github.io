...
- 낙하 이펙트는 착지 과정에서 단계적으로 사라지고 균열은 평범한 회색 선으로 남는다. idle 부유와 자동 낙하의 transform이 동시에 반대 방향으로 움직이지 않게 진폭을 감쇠해 연결한다.
