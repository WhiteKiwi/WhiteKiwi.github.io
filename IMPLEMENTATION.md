...
- 06↔07 전환은 document root에 전환 소유권을 표시해 같은 `window` 입력을 구독하는 05↔06 전환기가 동시에 시작되지 않게 한다. reveal이 끝난 뒤에는 마지막 wheel·touch·스크롤 키 입력부터 짧은 무입력 시간이 확보될 때까지 공통 잠금을 연장해, Contact에서 빠르게 역주행하며 남은 트랙패드 관성이 인접 경계를 다시 실행하지 않게 한다. 안정화가 끝날 때 스크롤 기준 위치와 touch 기준점을 함께 초기화한다.
...
- 벚꽃잎은 고정 seed에서 파생한 시작점·지연·속도·회전값으로 CSS 루프를 실행해 스크롤이 멈춰도 계속 흩날린다. 낙하 transform을 가진 안쪽 꽃잎과 포인터 바람 transform을 가진 바깥 wrapper를 분리한다. 데스크톱 fine pointer의 속도를 저역 통과 방식으로 부드럽게 만든 뒤 넓은 거리 감쇠의 방향성 gust와 화면 전체에 약하게 공유되는 residual wind를 만든다. 각 wrapper는 입자별 결합 강도로 같은 바람을 다르게 받아 실제 낙하 위치가 옆으로 누적 이동하며, 중심 복원력은 약하게 두고 drag와 잔류 바람을 매 프레임 적분해 입력 속도에 비례한 긴 여운을 남긴다. 커서로부터 바깥쪽으로 미는 회피 힘은 사용하지 않는다. 프레임 간 시간 차이를 사용해 고주사율에서도 속도를 일정하게 유지하며, 모션 감소 환경과 터치 포인터에서는 이 반응을 실행하지 않는다.
