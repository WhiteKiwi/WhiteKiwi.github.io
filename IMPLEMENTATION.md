...
- `whoami`는 셸의 `fortune`처럼 무작위 한 문장을 말풍선으로 먼저 보여준 뒤 정보 블록을 출력한다. 별도 명령으로 두지 않는다. 말풍선이 먼저 나오고 꼬리가 아래로 내려가 그 아래의 아스키 키위를 가리킨다. 말이 먼저, 말하는 쪽이 나중이어야 말풍선으로 읽힌다.
- 문장은 이 포트폴리오를 위해 직접 쓴 것만 사용한다. 셸에 설치된 fortune 데이터베이스는 제3자가 수집·번역한 인용문 모음이라 저장소로 옮기지 않는다.
- 말풍선 테두리와 줄바꿈은 문자 수가 아니라 칸 수로 계산한다. 좁은 화면에서는 더 짧은 폭으로 감싼다.
- 칸 수 계산만으로는 부족하다. DM Mono에는 한글이 없어 한글만 다른 글꼴로 떨어지는데, 실측 결과 그 글꼴의 한글 폭은 라틴의 1.44배였다. 2배가 아니므로 어떤 산술로도 격자를 맞출 수 없다. 아스키 격자가 필요한 줄에는 한글이 라틴의 정확히 2배인 `Nanum Gothic Coding`을 적용한다. 터미널의 나머지 출력은 DM Mono를 유지한다.
- 글꼴 서브셋이 내려오기 전 한 프레임은 대체 글꼴 폭으로 그려질 수 있다. Contact는 긴 여정의 끝이라 그 시점에는 이미 로드돼 있다.
...
- 반대로 상단 갱신 스탬프는 `resume-data.ts`의 상수로 손수 관리한다. 오늘 날짜로 찍으면 내용을 방치해도 최신인 척하게 되므로 자동 계산하지 않는다.
...
- 벚꽃잎은 고정 seed에서 파생한 시작점·지연·속도·회전값으로 CSS 루프를 실행해 스크롤이 멈춰도 계속 흩날린다. 낙하 transform을 가진 안쪽 꽃잎과 포인터 바람 transform을 가진 바깥 wrapper를 분리한다. 데스크톱 fine pointer의 입력은 작은 dead zone과 smoothstep 강도 곡선을 통과시킨 뒤 최근 약 0.4초의 위치·방향·속도·시각을 trail buffer에 일정 거리 간격으로 저장한다. 이는 OGL Flowmap이 속도를 감쇠하는 브러시 자국으로 누적하는 방식을 34개 DOM 입자에 맞게 이산화한 것이다. 매 프레임 각 꽃잎은 모든 유효 trail segment 중 가장 가까운 선분 투영점을 찾고, 거리 falloff·segment age·입력 속도를 곱한 벡터를 받아 해당 경로 방향으로 이동한다. 정지 시에는 새 segment를 만들지 않고 기존 segment만 시간에 따라 소멸한다. 비행 중 중심 복원력은 사용하지 않고 drag만으로 속도를 줄이며, 안쪽 꽃잎의 `animationiteration` 투명 경계에서 새 꽃잎처럼 wrapper 변위와 속도를 초기화해 장기 누적을 막는다. 프레임 간 시간 차이를 사용해 고주사율에서도 속도를 일정하게 유지하며, 모션 감소 환경과 터치 포인터에서는 이 반응을 실행하지 않는다.
...
- 프롤로그의 모든 색을 `--sky-*` 커스텀 속성 팔레트로 뽑아낸다. 값은 `rgb(var(--sky-light) / .4)` 형태로 쓸 수 있도록 RGB triplet으로 정의한다. 토큰은 배경 3단(`top`/`mid`/`bottom`), 페이지 바탕 `base`, 유리 하이라이트 `light`, 밝은 대기 `haze`, 유리 몸체 `tint`, 채도 높은 중간 파랑 `deep`, 그림자 `shade`, 안내 문구 `ink`다. 색이 아니라 세기를 다루는 토큰도 함께 둔다. `star`는 별 레이어 불투명도, `sun-alpha`는 해·달 글로우, `bloom`은 좌상단 대기 하이라이트, `ground`는 유리 지면 상단의 밝기다. 어두운 하늘에서 같은 흰색을 그대로 쓰면 낮의 하이라이트가 광원처럼 튀므로 이 네 개를 시간대별로 낮춘다.
- 팔레트는 `night`, `predawn`, `dawn`, `day`, `dusk` 다섯 개다. `:root`가 기본값으로 `day`를 정의하고 나머지 넷이 뒤에서 덮어쓴다. 로컬 시각 04—06시는 `predawn`, 06—08시는 `dawn`, 08—17시는 `day`, 17—20시는 `dusk`, 그 외는 `night`다.
- `predawn`은 노을이 아니라 해 뜨기 전의 하늘이다. 인디고에서 슬레이트 블루로 내려오다 지평선에만 옅은 온기를 남기고, 별은 밤보다 흐리게 유지한다. `dawn`은 라일락·로즈·크림의 차가운 아침 노을, `dusk`는 바이올렛·오렌지·골드의 따뜻한 저녁 노을로 서로 다른 온도를 준다.
- 시간대는 `FallIntro`가 마운트 시점에 한 번 정하고 `data-sky` 속성으로 노출한다. 첫 페인트에 팔레트가 적용되도록 `<main>`에 속성을 렌더하고, 프롤로그 스크롤 잠금 동안 `body` 배경도 같은 팔레트를 쓰도록 `document.documentElement`에도 같은 속성을 건다. `body` 인라인 배경은 하드코딩한 하늘색 대신 `var(--sky-lock)`을 쓴다.
- `?sky=night|predawn|dawn|day|dusk` 쿼리 파라미터로 팔레트를 강제할 수 있다. 어느 시간대든 실제 화면을 검토하기 위한 확인용 경로이며, 알 수 없는 값은 무시하고 현재 시각을 따른다.
- Milk Glass 알의 껍질·하이라이트·균열 색은 팔레트에서 제외한다. 주변 하늘이 어두워져도 알은 같은 흰색이라 밤에는 자연히 가장 밝은 물체가 된다.
- `night`와 `predawn`에만 별 레이어와 낮은 불투명도의 달빛을 추가한다. 별은 `.fall-intro::after`의 radial-gradient 점들과 느린 twinkle 루프다. blur가 걸린 `.sky-depth` 위에 두면 점이 뭉개지므로 장면 자체의 레이어로 두고, `predawn`에서는 곧 사라질 별처럼 더 흐리게 두고 모션 감소 설정에서는 정지한다.
