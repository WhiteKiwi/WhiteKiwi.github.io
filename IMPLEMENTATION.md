...
- 에이임팩트 후반에는 별도의 전환용 도형을 겹치지 않고 밭의 다섯 번째 작물인 실제 당근 DOM을 키보드 접근 가능한 버튼으로 사용한다. 스크롤 진행률은 클릭 가이드의 노출만 제어하고 당근 크기는 바꾸지 않는다. 클릭하면 시간 기반 CSS transition으로 약 0.45초 안에 당근 전체를 화면 크기로 확대하고, 가려진 프레임에서 당근 트랙 초입으로 즉시 스크롤한 뒤 동일한 `.crop.crop-1` 구조를 빠르게 축소해 지도를 드러낸다.
- 당근 트랙 초입에서 위 방향 스크롤이 임계 진행률을 통과하면 반대 흐름을 자동 실행한다. 당근 복제본을 빠르게 확대해 화면을 가린 뒤 에이임팩트의 클릭 지점으로 즉시 이동하고 실제 당근을 원래 크기로 축소한다. 전환 중에는 재진입을 잠그고 `prefers-reduced-motion`에서는 지연을 최소화한다.
