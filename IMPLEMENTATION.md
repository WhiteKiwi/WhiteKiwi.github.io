...
- 벚꽃잎은 고정 seed에서 파생한 시작점·지연·속도·회전값으로 CSS 루프를 실행해 스크롤이 멈춰도 계속 흩날린다. 낙하 transform을 가진 안쪽 꽃잎과 포인터 바람 transform을 가진 바깥 wrapper를 분리한다. 데스크톱 fine pointer의 속도를 저역 통과 방식으로 부드럽게 만들고 작은 dead zone과 smoothstep 강도 곡선을 통과시켜, 정지 시에는 새 힘이 0이고 느린 이동은 약하게만 반영한다. 넓은 거리 감쇠의 방향성 gust와 화면 전체에 약하게 공유되는 residual wind를 만들며, 각 wrapper는 입자별 결합 강도로 같은 바람을 다르게 받아 실제 낙하 위치가 옆으로 누적 이동한다. 비행 중 중심 복원력은 사용하지 않고 drag만으로 속도를 줄이며, 안쪽 꽃잎의 `animationiteration` 투명 경계에서 새 꽃잎처럼 wrapper 변위와 속도를 초기화해 장기 누적을 막는다. 커서로부터 바깥쪽으로 미는 회피 힘은 사용하지 않는다. 프레임 간 시간 차이를 사용해 고주사율에서도 속도를 일정하게 유지하며, 모션 감소 환경과 터치 포인터에서는 이 반응을 실행하지 않는다.
