...
- 화면이 가려진 프레임에서 당근 트랙 초입으로 즉시 위치를 바꾸되 당근 stage는 처음부터 정상 배율로 둔다. 그 위에 실제 뿌리와 같은 색의 full-viewport transition curtain을 놓고, 봄 교정 전환처럼 위로 천천히 걷어 올리며 opacity를 낮춰 동네 지도를 드러낸다. 역방향은 당근 초입의 상향 wheel·touch·키 입력을 native scroll 전에 가로채 curtain으로 먼저 화면을 가리고, 정상 배율의 에이임팩트 위치로 교체한 뒤 같은 curtain을 걷는다. 전환 중 입력을 잠그고 종료 시 양쪽 track의 임시 class를 모두 제거하며, `prefers-reduced-motion`에서는 지연을 최소화한다.
